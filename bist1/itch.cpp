                 ...
                 TPrice   baseBidPrice = (currTob[0].bid)/10 + bidswap;
                   printborder
                    ...
                    loadbidmemorymap();//Loading map to qoute Bid orders
                      printborder
                   // cout <<"lastfoundinprospectindex "<<BOLDRED<<(int)lastfoundinprospectindex<<RESET<<endl;
                 //   cout <<"firstfoundinprospectindex "<<BOLDRED<<(int)firstfoundinprospectindex<<RESET<<endl;
                  
                /*    for (uint8_t i = 0; i < QOUTEBIDORDERS; i++)
                    {
          cout << "\t foundInBidProspect[" << (int)i << "] = " <<(int) foundInBidProspect[i] <<  "\t prospectfoundInQuote[" << (int)i << "] = " <<(int) prospectfoundInQuote[i] << endl;
                    }
                    */
                  processFoundlings();
...
/*
 void printQuoteOrders( ) {
        for (int i = 0; i < QOUTEORDERS / 2; i++) {

             printOrderItemsSideBySide(i, quoteOrdersMap [mapID][i],  quoteOrdersMap [mapID][i + QOUTEORDERS / 2]);
    ...
    cout<<GREEN<<"processing foundlings"<<RESET<<endl;
      for (uint8_t p = 0; p < QOUTEBIDORDERS; p++)
                 ...
                 std::cout<< BOLDRED<<"prospect price "<<prospectBidPrice[p]<<" is not in the order array -> finding the first notfound order index "<<RESET<<endl;
                                ...
                                cout<<BOLDBLUE<<"\tcopying prospect price "<<RESET<<CYAN<< (int)p<<" to quoteorder "<<(int) i<<RESET<<", assuming qorder  is available"<<endl;          
